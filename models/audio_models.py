...
class CRNN_tagger(nn.Module):
    ...
    def __init__(self, args=False, num_classes=cfg.class_num, learn_method=cfg.learn_method):
    ...
    def forward(self, x):
        ...
        # output dense
        if (self.learn_method == 'Contrastive') or (cfg.premodel_ext):
            x1 = self.dense(x)
            return x1, x
        else:
            x = self.dense(x)
...
if __name__ == '__main__':
    ...
    batchsize = 2
    x = torch.randn(size=(batchsize, 1, 128, 304))
    model = CRNN_tagger() # MobileNetV2()
    output_z = model(x)  # , output_recon
    print(f'input dim:{x.shape}. output_z dim:{output_z[1].shape}.')
